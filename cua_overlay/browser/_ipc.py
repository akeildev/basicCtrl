...
# CUA_BROWSER_TMP_DIR set → caller-isolated dir, bare filenames (avoids AF_UNIX sun_path
# overrun: 104 macOS / 108 Linux). Unset → shared tmpdir, "cuabr-<NAME>" prefix
...
CUA_BROWSER_TMP_DIR = os.environ.get("CUA_BROWSER_TMP_DIR")
_TMP = Path(CUA_BROWSER_TMP_DIR or (tempfile.gettempdir() if IS_WINDOWS else "/tmp"))
...
def _check(name):  # path-traversal guard for CUA_BROWSER_NAME
    if not _NAME_RE.match(name or ""):
        raise ValueError(f"invalid CUA_BROWSER_NAME {name!r}: must match [A-Za-z0-9_-]{{1,64}}")
    return name


def _stem(name):  # "bu" when CUA_BROWSER_TMP_DIR isolates us, else "cuabr-<NAME>"
    _check(name)
    return "bu" if CUA_BROWSER_TMP_DIR else f"cuabr-{name}"

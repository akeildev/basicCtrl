"""CDP WS holder + IPC relay (Unix socket on POSIX, TCP loopback on Windows). One daemon per CUA_BROWSER_NAME."""
...
def _load_env():
    ...
    workspace = Path(os.environ.get("CUA_BROWSER_WORKSPACE", repo_root / "agent-workspace")).expanduser()
...
NAME = os.environ.get("CUA_BROWSER_NAME", "default")
...
REMOTE_ID = os.environ.get("CUA_BROWSER_ID")
...
def get_ws_url():
    if url := os.environ.get("CUA_BROWSER_CDP_WS"):
        return url
    if url := os.environ.get("CUA_BROWSER_CDP_URL"):
        ...
        while time.time() < deadline:
            try:
                ...
                last_err = e
                time.sleep(1)
        raise RuntimeError(f"CUA_BROWSER_CDP_URL={url} unreachable after 30s: {last_err} -- is the dedicated automation Chrome running?")
    ...
    for probe_port in (9222, 9223):
        try:
            ...
            continue
    raise RuntimeError(f"DevToolsActivePort not found in {[str(p) for p in PROFILES]} — enable chrome://inspect/#remote-debugging, or set CUA_BROWSER_CDP_WS for a remote browser")
...
class Daemon:
    ...
    async def start(self):
        ...
        try:
            ...
            if os.environ.get("CUA_BROWSER_CDP_WS"):
                raise RuntimeError(

...
human_verification:
  - test: "Calculator click <50ms via L0 push (SC-1)"
    expected: "uv run python -m basicctrl.demo.calculator_click exits 0 with VERIFIED, latency_ms<50, L0 signal=1.0 (or L1 carries per renormalization), L2=None, L3=None"
...
| -------- | -------- | ------ | ------- |
| `basicctrl/__init__.py` | Package marker, version | ✓ VERIFIED | Exists, `__version__ = "0.1.0"`. |
...
| `basicctrl/state/graph.py` | UIElement Pydantic v2 model + StateGraph + EdgeKind | ✓ VERIFIED | All 22 UIElement fields present per ARCHITECTURE.md L40-49. |
| `basicctrl/state/causal_dag.py` | ActionCanonical (kind: Literal["READ","MUTATE"]), HoarePre, HoarePost, CausalDAG | ✓ VERIFIED | All Pydantic v2 contracts locked. HoarePost.verified == confidence>=0.5 enforced by model_validator. |
| `basicctrl/state/ring_buffer.py` | TemporalRingBuffer (deque maxlen=5) + StateSnapshot | ✓ VERIFIED | maxlen=5, PRECEDES edges between consecutive frames sharing keys. |
| `basicctrl/state/fingerprint.py` | composite_key tier ladder | ✓ VERIFIED | 3-tier ladder: axid > path > bbox (4px-grid centroid). |
| `basicctrl/profile/classifier.py` | AppProfile + classify async fn | ✓ VERIFIED | Pydantic AppProfile with 15 fields + cache_key property; classify uses anyio.create_task_group + per-probe 200ms timeout. |
| `basicctrl/profile/cache.py` | Atomic disk cache + version-invalidation | ✓ VERIFIED | `os.replace`-based atomic write; should_invalidate_cache compares bundle_version + bundle_build. |
| `basicctrl/profile/tcc.py` | TCCMonitor with .check() + on_revocation() | ✓ VERIFIED | AXIsProcessTrusted lazy-imported; on_revocation emits structlog `tcc_revoked` event with action URL + raises SystemExit(2). |
| `basicctrl/ax/rate_limit.py` | TokenBucket(20/sec/pid) | ✓ VERIFIED | rate_per_sec=20.0, capacity=20; per-pid asyncio.Lock; structlog `ax.rate_limited` deny event. |
| `basicctrl/ax/walker.py` | walk_subtree(max_depth=3, max_children=50, max_nodes=500) | ✓ VERIFIED | Iterative BFS (work-queue, no Python recursion); WalkResult.truncated + cap_hit. |
| `basicctrl/ax/modal_probe.py` | has_blocking_modal | ✓ VERIFIED | Top-10 windows scan via AXModal; uses TokenBucket. |
| `basicctrl/ax/errors.py` | Typed AX exception hierarchy | ✓ VERIFIED | 6 subclasses sourced from live HIServices kAXError* exports. |
| `basicctrl/ax/observer.py` | AXEventBridge (CFRunLoop thread + asyncio Queue) | ⚠️ ORPHANED | Exists, wired, but WR-01 + WR-03 + WR-04 (see Gaps). Subscription works for single-element single-pid demo path. |
| `basicctrl/verifier/axobserver.py` | AXObserverManager.expect (5ms guard) | ✓ VERIFIED | _passes_filter uses 5_000_000ns guard; 8 unit tests cover all 4 predicates + dispatcher. |
| `basicctrl/verifier/nsworkspace.py` | NSWorkspaceObserver | ✓ VERIFIED | NSWorkspaceDidActivateApplicationNotification on dedicated NSOperationQueue. |
| `basicctrl/verifier/kqueue_proc.py` | KqueueProcObserver (EVFILT_PROC + NOTE_EXIT) | ✓ VERIFIED | Pure asyncio via loop.add_reader; __aenter__/__aexit__ for fd-leak resistance (T-1-06). |
| `basicctrl/verifier/distnotif.py` | DistributedNotificationEvent contract scaffold | ✓ VERIFIED | Pydantic frozen contract; observer is Phase 2 stub raising NotImplementedError. |
| `basicctrl/verifier/ensemble/l0_push.py` | L0Push consumer (no polling) | ✓ VERIFIED | Source-grep confirms 0 occurrences of walk_subtree / AXUIElementCopyAttributeValue / read_attribute. All 7 AX notif types mapped to signals. |
| `basicctrl/verifier/ensemble/l1_cheap.py` | L1Cheap (CGWindowList + NSPasteboard.changeCount + dHash) | ✓ VERIFIED | Three sub-checks in anyio.create_task_group; T-1-03 mitigation: pasteboard signal is integer-only (verified by `test_no_pasteboard_contents_logged`). |
| `basicctrl/verifier/ensemble/l2_medium.py` | L2Medium (Vision OCR + walker delegation) | ✓ VERIFIED | walk_subtree with default caps (max_depth=3); 0 occurrences of AXUIElementCopyAttributeValue or max_depth>=4 in L2 module. |
| `basicctrl/verifier/ensemble/l3_llm.py` | L3Stub raising NotImplementedError | ✓ VERIFIED | @runtime_checkable Protocol; catch-all *args, **kwargs signature; 3 unit tests. |
| `basicctrl/verifier/ensemble/weighted_vote.py` | WeightedVote with present-signal renormalization | ✓ VERIFIED | Active = signals with non-zero value; weighted_sum / active_total renormalization. VERIFIED_THRESHOLD=0.50, L3_ESCALATE_THRESHOLD=0.30 module constants. |
| `basicctrl/verifier/aggregator.py` | Aggregator (L0+L1 parallel + L2/L3 escalation) | ✓ VERIFIED | anyio.create_task_group for L0+L1; L2 boost (0.2 * max signal, halved if walker_truncated); L3 catch with `l3.unavailable_phase1` warning. |
| `basicctrl/persist/session_writer.py` | SessionWriter (~/.cua/sessions/<uuid>/) | ✓ VERIFIED | UUID4 session_id; 5 subdirs (checkpoints/recipes/cassettes/recordings/profile_snapshot) + heals.ndjson + action_log.ndjson + atomic snapshot.json. |
| `basicctrl/persist/durable_step.py` | DurableExecutor (AsyncPostgresSaver wrap) | ✓ VERIFIED | from_conn_string async context manager; setup() / checkpoint() / latest_checkpoint() / aclose(); _mask_conn for T-1-02 (per WR-05 over-redacts harmless cases — see Info). |
| `basicctrl/persist/resume.py` | resume_from_checkpoint + ResumeContext | ✓ VERIFIED | Returns None for fresh sessions; ActionCanonical.model_validate round-trips through Postgres. |
| `basicctrl/mcp_server/main.py` | FastMCP bootstrap + cua-driver stdio spawn | ✓ VERIFIED | StdioServerParameters(command="cua-driver", args=["mcp"]); run_stdio_async; full Phase 1 stack wired. T-1-01 mitigated. |
| `basicctrl/mcp_server/proxy.py` | register_proxied_tool + ACTION_CLASS_TOOLS | ✓ VERIFIED | 10 upstream tools mapped to 4 canonical action classes (click/scroll/type/set_value); PRE/FIRE/POST/LOG/CHECKPOINT wrap. |
| `basicctrl/mcp_server/healing_tools.py` | click_with_healing | ✗ STUB (docstring lies) | Tool registered but bypasses verifier wrap (WR-02). See Gaps. |
| `basicctrl/demo/calculator_click.py` | run_demo() coroutine + main() CLI | ✓ VERIFIED | run_demo() returns 10-field result dict; main() pretty-prints with rich; PHASE-1-DEMO.md operator runbook present. |
| `scripts/init_postgres.sh` | Idempotent provisioning | ✓ VERIFIED | createdb basicctrl (literal match); idempotent (skips if DB exists); `psql basicctrl -c '\dt' | grep -c checkpoint` returns 4. |
...
| Public-API import smoke | `python -c "from basicctrl.verifier import Aggregator, L0Push, L1Cheap, L2Medium, L3Stub, WeightedVote, VERIFIED_THRESHOLD"` | exits 0 | ✓ PASS |
| MCP module entry runnable | `python -c "import basicctrl.mcp_server.__main__"` | exits 0 | ✓ PASS |
| No TCP transport in MCP proxy | `grep -rE "socket\.AF_INET|listen\(|\.bind\(" basicctrl/mcp_server/` | 0 matches | ✓ PASS |
| All 7 AX notifications wired | `grep "AX...Changed\|AX...Created" basicctrl/verifier/ensemble/l0_push.py` | 7 matches (Value/Focused/Window/Title/Layout/SelectedText/SelectedRows) | ✓ PASS |
| Swift cua-driver untouched | `git diff --name-only main..HEAD libs/cua-driver/Sources/` | empty | ✓ PASS |
| Calculator demo (live) | `uv run python -m basicctrl.demo.calculator_click` | Per PHASE-1-DEMO.md: verified=True, latency 31-45ms, L0 sometimes flakes (L1 carries) | ? SKIP (requires TCC + Calculator) |
| Postgres tables provisioned | `psql basicctrl -c '\dt' | grep -c checkpoint` | per Plan 07 SUMMARY: 4 tables | ? SKIP (requires postgres up) |

### Requirements Coverage (18 requirement IDs from ROADMAP)

| Requirement | Source Plan | Description | Status | Evidence |
| ----------- | ----------- | ----------- | ------ | -------- |
| CORE-01 | 01-01 | Python overlay scaffold above libs/cua-driver/ | ✓ SATISFIED | `basicctrl/` exists; `libs/cua-driver/` vendored; pyproject.toml pins all deps. |
...
| CORE-03 | 01-02 | bundleID → AppProfile classifier | ✓ SATISFIED | `basicctrl/profile/classifier.py::classify`; per-bundle disk cache; survives session restart. |
...
| ---- | ---- | ------- | -------- | ------ |
| basicctrl/mcp_server/healing_tools.py | 56-63 + 94 | Docstring claims verifier ladder runs; implementation bypasses it | ⚠️ Warning (WR-02) | Audit-trail incomplete for healing-named calls; MCP host introspection sees misleading description. |
| basicctrl/ax/observer.py | 255-307 | Per-pid callback closure leaks stale element_key/pid to subsequent subscribers | ⚠️ Warning (WR-01) | Phase 1 demo invisible (single subscribe per session); Phase 2 race orchestrator hits this. AXEvent.element_key field is misleading data after first subscribe. |
| basicctrl/ax/observer.py | 256-274 | Callback can raise RuntimeError into PyObjC C boundary on loop close | ⚠️ Warning (WR-04) | Race window during stop() ; potential SIGABRT on stress shutdown. |
| basicctrl/ax/observer.py | 108-112, 247-248, 307 | _callbacks/_refcon_to_action/_subscriptions grow without bound | ⚠️ Warning (WR-03) | Phase 1 short sessions invisible; Phase 3 high-frequency action throughput accumulates ~100 bytes per action. Refcon collision risk grows monotonically (WR-04 IN-01 4-billion bound). |
| basicctrl/persist/durable_step.py | 161-169 | _mask_conn over-redacts conn strings without password (false positive on `user@host:5432`) | ⚠️ Warning (WR-05) | Operators debugging "wrong host?" see *** instead of useful info. T-1-02 still holds (no false negative); UX problem only. |
| basicctrl/verifier/kqueue_proc.py | 47 | asyncio.get_event_loop() fallback emits DeprecationWarning in Python 3.12+ | ⚠️ Warning (WR-06) | Today: callers always pass loop=loop explicitly. Future: Python 3.14 may raise where 3.12 warns. |
...
**Test:** Run `uv run python -m basicctrl.demo.calculator_click` after granting Accessibility TCC to the Python interpreter.
...
**Test:** First run: `uv run python -m basicctrl.demo.calculator_click`. Second run within same session.
...
**Test:** Three terminals: (a) `uv run python -m basicctrl.demo.calculator_click`; (b) `kill -9 <pid>` while Postgres checkpoint is being written; (c) call `resume_from_checkpoint(session_id, durable)` for the same session_id.

...
must_haves:
  truths:
    - "basicctrl/translators/base.py defines Translator Protocol with tier: Literal['T1','T2','T3','T4','T5'] and async resolve(bundle_id, pid, target_spec) -> Optional[TranslatorTarget] and async validate(target) -> bool"
    - "basicctrl/translators/base.py defines TranslatorTarget Pydantic model with fields: element (UIElement), ax_element (Optional[Any]), cdp_node_id (Optional[int]), cdp_session_id (Optional[str]), as_target_spec (Optional[str]), grounded_bbox (Optional[Bbox]), extras (dict)"
    - "basicctrl/translators/base.py defines TargetSpec Pydantic model with fields: x, y, label, role, aria_label, css (all Optional), key (str), as_verb (Optional[str])"
    - "basicctrl/translators/registry.py defines TranslatorRegistry with register(translator), get(tier) -> Translator, select_for_priority(priority_list) -> list[Translator]"
    - "basicctrl/actions/channels/base.py defines Channel Protocol with name: Literal['C1','C2','C3','C4','C5'] and async fire(action, target, store, cancel_event) -> ChannelOutcome"
    - "basicctrl/actions/channels/base.py defines ChannelOutcome Pydantic frozen model with fields: channel (Literal C1-C5), status (Literal 'fired'|'skipped'|'cancelled'|'errored'), fired_at_ns (Optional[int]), error (Optional[str]), verified (bool, default False)"
    - "basicctrl/actions/channel_registry.py defines ChannelRegistry with register(channel), get(name) -> Channel, select(translator_priority, race_policy) -> list[Channel] using D-14 default mapping (T1→C2, T2→C5, T3→C4, T4→C1, T5→C3)"
    - "ChannelRegistry.tier_for_channel(channel_name) returns the default tier (T1-T5) for a registered channel name (C1-C5) via inverted TIER_TO_CHANNEL_DEFAULT lookup; returns None for unknown channels"
  ...
  artifacts:
    - path: "basicctrl/translators/__init__.py"
    ...
    - path: "basicctrl/translators/base.py"
    ...
    - path: "basicctrl/translators/registry.py"
    ...
    - path: "basicctrl/actions/channels/base.py"
      provides: "Channel Protocol + ChannelOutcome Pydantic frozen model"
      contains: "class Channel|class ChannelOutcome"
    - path: "basicctrl/actions/channel_registry.py"
      provides: "ChannelRegistry + D-14 default translator→channel binding"
      contains: "class ChannelRegistry|TIER_TO_CHANNEL_DEFAULT|CHANNEL_TO_TIER_DEFAULT|tier_for_channel"
  key_links:
    - from: "basicctrl/actions/channel_registry.py:ChannelRegistry.select"
      to: "basicctrl/actions/channels/base.py:Channel.fire"
    ...
    - from: "basicctrl/translators/registry.py:TranslatorRegistry.select_for_priority"
      to: "basicctrl/translators/base.py:Translator.resolve"
...
Output:
- `basicctrl/translators/{__init__.py, base.py, registry.py}`
- `basicctrl/actions/channels/{__init__.py, base.py}`
- `basicctrl/actions/channel_registry.py`
...
<task type="auto">
  ...
  <action>
**Step 1.** Create `basicctrl/translators/__init__.py`:
...
**Step 2.** Create `basicctrl/translators/base.py`:
...
**Step 3.** Create `basicctrl/translators/registry.py`:
...
DO NOT add tier="T0" or "T6" to the Literal type (locked at T1..T5 per requirements).
  ...
  <acceptance_criteria>
- `basicctrl/translators/base.py` contains literal `class Translator(Protocol)`
- `basicctrl/translators/base.py` contains literal `class TranslatorTarget`
- `basicctrl/translators/base.py` contains literal `class TargetSpec`
- `basicctrl/translators/base.py` contains literal `Literal["T1", "T2", "T3", "T4", "T5"]`
- `basicctrl/translators/registry.py` contains literal `class TranslatorRegistry`
- `basicctrl/translators/registry.py` contains literal `select_for_priority`
- `basicctrl/translators/__init__.py` re-exports `Translator`, `TranslatorTarget`, `TargetSpec`, `TranslatorRegistry`
...
<task type="auto">
  ...
  <read_first>
    - basicctrl/state/causal_dag.py (ActionCanonical schema)
    - basicctrl/translators/base.py (TranslatorTarget — channel.fire takes one)
    - basicctrl/actions/idempotency.py (IdempotencyTokenStore — channel.fire takes one)
    - basicctrl/actions/race_policy.py (RacePolicy enum)
  ...
  <action>
**Step 1.** Create `basicctrl/actions/channels/__init__.py`:
...
**Step 2.** Create `basicctrl/actions/channels/base.py`:
...
**Step 3.** Create `basicctrl/actions/channel_registry.py`:
...
DO NOT bind T1→C5 or change the D-14 mapping. DO NOT make ChannelOutcome mutable.
  ...
  <acceptance_criteria>
- `basicctrl/actions/channels/base.py` contains literal `class Channel(Protocol)`
- `basicctrl/actions/channels/base.py` contains literal `class ChannelOutcome`
- `basicctrl/actions/channels/base.py` contains literal `Literal["C1", "C2", "C3", "C4", "C5"]`
- `basicctrl/actions/channels/base.py` contains literal `frozen=True`
- `basicctrl/actions/channel_registry.py` contains literal `class ChannelRegistry`
- `basicctrl/actions/channel_registry.py` contains literal `TIER_TO_CHANNEL_DEFAULT`
- `basicctrl/actions/channel_registry.py` contains the verbatim mapping `"T1": "C2"` (and all 5 D-14 entries)
- `basicctrl/actions/channel_registry.py` contains literal `CHANNEL_TO_TIER_DEFAULT`
- `grep 'def tier_for_channel' basicctrl/actions/channel_registry.py` returns 1 match
- `uv run pytest -q tests/unit/actions/test_channel_registry.py::test_tier_for_channel_inverse_lookup` exits 0
- `uv run pytest -q tests/unit/actions/test_channel_registry.py` exits 0 with 11 passed
  </acceptance_criteria>
...
<verification>
- `uv run pytest -q tests/unit/translators/test_translators_registry.py tests/unit/actions/test_channel_registry.py` exits 0 with 16 passed (5 + 11)
- `python -c "from basicctrl.translators import Translator, TranslatorTarget, TargetSpec, TranslatorRegistry; from basicctrl.actions.channels.base import Channel, ChannelOutcome; from basicctrl.actions.channel_registry import ChannelRegistry, TIER_TO_CHANNEL_DEFAULT; print('ok')"` prints `ok`

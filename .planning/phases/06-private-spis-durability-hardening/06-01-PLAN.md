...
depends_on: []
files_modified:
  - cua_overlay/spi/__init__.py
  - cua_overlay/spi/probe.py
...
must_haves:
  ...
  artifacts:
    - path: "cua_overlay/spi/__init__.py"
      provides: "SPI module entry point + pytest.importorskip gate"
    - path: "cua_overlay/spi/probe.py"
...
<task type="auto">
  ...
  <files>cua_overlay/spi/__init__.py, cua_overlay/spi/probe.py</files>
  <read_first>
    - File being modified: cua_overlay/spi/ (new package)
    ...
    - cua_overlay/log.py (structlog patterns for INFO logging)
  </read_first>
  <action>
Create cua_overlay/spi/__init__.py as module init with pytest.importorskip gate:
...
Create cua_overlay/spi/probe.py with probing logic for all 8 SPIs:
...
```
  ...
  <done>
    cua_overlay/spi/__init__.py and cua_overlay/spi/probe.py created and tested. All 8 probe functions implemented with graceful fallbacks. SPICapabilities dataclass locked. Tests pass.
  </done>
</task>

<task type="auto">
  <name>Task 2: Extend AppProfile with SPI availability fields</name>
  <files>cua_overlay/profile/classifier.py</files>
  <read_first>
    - File being modified: cua_overlay/profile/classifier.py (existing Phase 1-5)
    - RESEARCH.md §"Capability Probe Pattern" L181-217 (AppProfile.spi_*_available pattern)
    - cua_overlay/profile/classifier.py (existing AppProfile structure)
    - Phase 4 cognition_capable pattern (how to extend AppProfile with new fields)
  </read_first>
  <action>
Extend cua_overlay/profile/classifier.py AppProfile dataclass to add 8 SPI capability fields.
...
Update profile_cache.py to persist SPI capability flags to disk cache (same as cognition_capable from Phase 4).
  ...
  <verify>
    <automated>
      ...
      grep -c "spi_skylight_available" cua_overlay/profile/classifier.py &&
      grep -c "probe_spi_capabilities" cua_overlay/profile/classifier.py
    </automated>
  </verify>
  <done>
...
<success_criteria>
- [ ] cua_overlay/spi/__init__.py + cua_overlay/spi/probe.py created and importable
...
- [ ] Grep verifiable: `grep -c "spi_.*_available" cua_overlay/profile/classifier.py >= 8`
- [ ] Grep verifiable: `grep -c "def probe_" cua_overlay/spi/probe.py >= 8`

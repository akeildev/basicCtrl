...
VERSION_CACHE = Path(tempfile.gettempdir()) / "cuabr-version-cache.json"
...
def _is_local_chrome_mode(env=None):
    ...
    return not (env or {}).get("CUA_BROWSER_CDP_WS") and not os.environ.get("CUA_BROWSER_CDP_WS")
...
def _daemon_endpoint_names():
    # CUA_BROWSER_TMP_DIR isolates one daemon per dir → no filename-prefix discovery,
    # just check whether our local endpoint exists. Without CUA_BROWSER_TMP_DIR, _TMP
    ...
    if ipc.CUA_BROWSER_TMP_DIR:
        return [NAME] if (ipc._TMP / f"cuabr{suffix}").exists() else []
    names = []
    for p in sorted(ipc._TMP.glob(f"cuabr-*{suffix}")):
...
def start_remote_daemon(name="remote", profileName=None, **create_kwargs):
    ...
    try:
        ensure_daemon(
            name=name,
            env={"CUA_BROWSER_CDP_WS": _cdp_ws_from_url(browser["cdpUrl"]), "CUA_BROWSER_ID": browser["id"]},
        )
    except BaseException:
